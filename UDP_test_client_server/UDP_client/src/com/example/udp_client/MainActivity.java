...
public class MainActivity extends Activity {
	...
	String ipstring;
	int portsend;
	FindServerThreadAllJoyn mSendThread=null;
	Button listen;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mSendThread=new FindServerThreadAllJoyn();
		mSendThread.init();
...
//		ipstring=mSendThread.GetServerIPadress(temp);
	 ...
	 class FindServerThreadAllJoyn extends Thread {
		 ...
		 {   String strPing=null;
			 try {
				   ...
				   strPing = mSimpleInterface.Ping(buff);
			 ...
			 while(go)
			 {
				  synchronized(FindServerThreadAllJoyn.this){
					 ...
					 Status status = mBus.joinSession(NAME, 
				      ...
				      ipstring=GetServerIPadress("Request IP");
			 }
		 }
	 }
     class SendThreadEfflux extends Thread
     {   MultiParticipantSession mMultiSession;
      	 String Host;
      	 int Port;
      	 RtpParticipant mParticipant;
      	 int ssrc;
     	 public SendThreadEfflux(String host,int port)
     	 {
     		 this.Host=host;
     		 this.Port=port;
     	 }
    	 public void init()
    	 {
    	     RtpParticipant participant = RtpParticipant
                     .createReceiver(new RtpParticipantInfo(ssrc), Host, Port, Port+1);
    	 }
